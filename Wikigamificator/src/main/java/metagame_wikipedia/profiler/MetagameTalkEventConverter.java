...
public class MetagameTalkEventConverter extends MetagameEventConverter implements CompleteWikitextVisitorNoReturn{

	
	

	public MetagameTalkEventConverter(WikiConfig config, int wrapCol, List<MetagameEvent> events, DiffAction action,
			Date timestamp, Integer playerId, String playerEmail, Integer articleNs, String page, Map<String, String> proyects) {
		super(config, wrapCol, events, action, timestamp, playerId, playerEmail, articleNs,page.replace("Talk:", ""),proyects);
		this.page=page;
	}
...
//		System.out.println("Section");
	...
	public void visit(WtParagraph p)
	{
		...
		events.add(new MetagameEvent(playerEmail, 
				this.getEventType("Paragraph"), action, 
				"Paragraph", timestamp,page,proyect));
		iterate(p);
...
//		System.out.println("Image");
	...
	public void visit(WtSignature n) {
		iterate(n);
		events.add(new MetagameEvent(playerEmail, 
				this.getEventType("Signature"), action, 
				"Signature", timestamp,page,proyect));
...
//			System.out.println("number of : = "+StringUtils.countOccurrencesOf(n.getRtd().toString(), ":"));
			events.add(new MetagameEvent(playerEmail, 
					this.getEventType("Answer"), action, 
					"Answer "+StringUtils.countOccurrencesOf(n.getRtd().toString(), ":"),
	...
	public void visit(WtPageName n) {
		if (n.getAsString().contains("User:")) {
			events.add(new MetagameEvent(playerEmail, 
					this.getEventType("User"), action, 
					"User",timestamp,page,proyect));
		}
